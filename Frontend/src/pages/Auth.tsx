...
export function Auth({ onAuth }: { onAuth: (token: string, user: User) => void }) {
  ...
  const [f, setF] = useState({ username: '', email: '', password: '', organizationId: '' });
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const r = mode === 'login'
        ...
        : await Api.register({ username: f.username, email: f.email, password: f.password, organizationId: f.organizationId || undefined });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <form onSubmit={submit} className="bg-white shadow rounded-lg p-8 w-96 space-y-3">
        ...
        <p className="text-slate-500 text-sm">{mode === 'login' ? 'Sign in to join a contest' : 'Create a learner account'}</p>
        ...
        {mode === 'register' && <input className={input} placeholder="Organization ID (optional)" value={f.organizationId} onChange={(e) => setF({ ...f, organizationId: e.target.value })} />}
